      ...
      if (isOrder && origin && origin.id_order) {
        ...
        if (order && order.order_details) {
          ...
          if (response.success && response.content) {
            setManualPdfDataUrl(
              `data:application/pdf;base64,${response.content}`
            );
            setPrintOptionModalVisible(true);
          } else {
            console.error("Error al generar ticket:", response.message);
          }
        } else {
    ...
    <>
                    ...
                    >
                      ...
                      <Column
                        ...
                        body={(rowData) => {
                          ...
                          if (type === "venta") {
                            icons.push(
                              <i key="print" className="pi pi-print mr-1" />
                            );
                            ...
                            icons.push(
                              <i key="undo" className="pi pi-undo mr-1" />
                            );
                          } else if (type === "entrada") {
                            icons.push(
                              <i key="in" className="pi pi-download mr-1" />
                            );
                          } else if (type === "salida") {
                            icons.push(
                              <i key="out" className="pi pi-upload mr-1" />
                            );
                          } else if (type === "traspaso") {
                            icons.push(
                              <i
                                key="move"
                                className="pi pi-arrow-right-arrow-left mr-1"
                              />
                            );
                          }
                          return (
      ...
      >
        <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
          <span>¿Deseas ver el ticket o imprimirlo?</span>
          <div
            style={{
              display: "flex",
          ...
          >
            <Button icon="pi pi-eye" label="Ver" onClick={handleViewTicket} />
            <Button
              icon="pi pi-print"
              label="Imprimir"
